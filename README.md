...
Use `/creator_notif creator_username message channel` to watch a creator. Creator IDs are treated as TikTok usernames, so `examplecreator` and `@examplecreator` both work. The creator field autocompletes from the imported database; omit it or choose `random` to pick a random imported creator. Live notifications are currently routed to Discord channel `1512190588009582753`.
